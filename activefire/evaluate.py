...
for mask_file, pred_file in zip(txts_mask_path, txts_pred_path):
    y_true = np.loadtxt(mask_file)
    y_pred = np.loadtxt(pred_file)

    if y_pred.shape != y_true.shape:
        y_pred_resized = zoom(y_pred, (y_true.shape[0] / y_pred.shape[0], y_true.shape[1] / y_pred.shape[1]))
        np.savetxt(pred_file, y_pred_resized, fmt='%i')
        print(f"Resized {pred_file} from {y_pred.shape} to {y_pred_resized.shape}")
...
for txt_mask_path, txt_pred_path in zip(txts_mask_path, txts_pred_path):
    try:
        ...
        y_true = np.loadtxt(txt_mask_path)
        y_pred = np.loadtxt(txt_pred_path)

        if y_true.shape != (IMAGE_SIZE[0], IMAGE_SIZE[1]) or y_pred.shape != (IMAGE_SIZE[0], IMAGE_SIZE[1]):
            skipped_samples += 1
            continue
...
print(f"Skipped {skipped_samples} samples due to shape mismatch.") 
